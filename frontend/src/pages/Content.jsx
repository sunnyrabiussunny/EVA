...
import { Sparkles, Trash2, Check, Archive, X, Copy, RefreshCw } from 'lucide-react';
    ...
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal">
        ...
        <div className="modal-body">
          ...
          <div className="form-row">
            <label className="form-label">Angle (optional — auto if empty)</label>
            <select value={angle} onChange={e => setAngle(e.target.value)}>
              <option value="">Random / AI decides</option>
              {ANGLES.map(a => <option key={a} value={a}>{a}</option>)}
            </select>
          </div>
          <div className="form-row">
            <label className="form-label">Topic hint (optional)</label>
            <input value={topic} onChange={e => setTopic(e.target.value)} placeholder="e.g. self-hosted AI, building in public, Bangladesh tech..." />
          </div>
    ...
    <div className="card">
      ...
      <div style={{ fontSize: 13, color: 'var(--text2)', lineHeight: 1.65, whiteSpace: 'pre-wrap' }}>
        ...
        {!expanded && item.content?.length > 200 && (
          <button onClick={() => setExpanded(true)} style={{ background: 'none', border: 'none', color: 'var(--accent2)', cursor: 'pointer', fontSize: 12 }}> ...show more</button>
        )}
        {expanded && <button onClick={() => setExpanded(false)} style={{ background: 'none', border: 'none', color: 'var(--accent2)', cursor: 'pointer', fontSize: 12 }}> show less</button>}
      </div>
    ...
    <div>
      <div className="page-header">
        <div>
          ...
          <div className="page-sub">{items.filter(i => i.status === 'ready').length} ready to publish</div>
        </div>
        <div className="flex-center gap-10">
          <button className="btn btn-ghost" onClick={() => generate({ platform: 'linkedin' })} disabled={loading}>
            <RefreshCw size={14} className={loading ? 'spin' : ''} /> Quick Generate
          </button>
      ...
      <div className="page-body">
        ...
        {filtered.length === 0
          ? <div className="empty"><div className="empty-icon">✍️</div><div className="empty-title">No {filter} content</div><div className="empty-sub">Generate content to fill your queue</div></div>
          : <div className="grid grid-auto" style={{ gap: 14 }}>
              {filtered.map(item => <ContentCard key={item.id} item={item} onStatus={setStatus} onDelete={del} toast={toast} />)}
            </div>
        }
      </div>
